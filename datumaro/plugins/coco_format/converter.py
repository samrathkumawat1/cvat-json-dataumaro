...
class _TaskConverter:
    ...
    def save_image_info(self, item, filename):
        ...
        #     'width': int(w),
        #     'height': int(h),
        #     'Image': cast(filename, str, ''),
        # })

    # def save_categories(self, dataset):
    #     raise NotImplementedError()

    def save_annotations(self, item , label_data):
        raise NotImplementedError()
...
class _CaptionsConverter(_TaskConverter):
    ...
    def save_annotations(self, item , label_data):
        for ann_idx, ann in enumerate(item.annotations):
            ...
            yolo_bb = _make_yolo_bbox((width, height), bbox)
...
class _InstancesConverter(_TaskConverter):
    ...
    def save_annotations(self, item ,label_data ):
        ...
        hat = self.annotations
        hatbc = self._data
        for instance in instances:
            elem = self.convert_instance(instance, item ,label_data)
            if elem:
                self.annotations.append(elem)

    def convert_instance(self, instance, item ,label_data):
        ...
        self._data['Image'] = item.id + item.image.ext
        self._data['Image_width'] = image_width
        self._data['Image_height'] = image_height
        ...
        yolo_bb = _make_yolo_bbox(( image_width, image_height), ann.points)
        yolo_bb = ' '.join('%.6f' % p for p in yolo_bb)
        yolo_bb_annotation += '%s %s' % (ann.label, yolo_bb)
        ...
        if self._context._allow_attributes:
                attrs = self._convert_attributes(ann)
                if attrs:
                    if attrs.get('Blured') == True:
                        attrs['Blured'] = "Yes"
                    else:
                        attrs['Blured'] = "No"

                    if attrs.get('Truncation') == True:
                        attrs['Truncation'] = "Yes"
                    else:
                        attrs['Truncation'] = "No"

                    if attrs.get('Occulation') == True:
                        attrs['Occulation'] = "Yes"
                    else:
                        attrs['Occulation'] = "No"

                    if attrs.get('Orientation') != None:
                        attrs['Orientation'] = str(int(attrs.get('Orientation')))
        ...
        elem['Identity'] = Identity
        elem['Yolo_Data'] = yolo_bb_annotation
        return elem
...
class CocoConverter(Converter):
    ...
    def apply(self):
        ...
        for subset_name, subset in self._extractor.subsets().items():
            ...
            label_data = subset.categories().get(AnnotationType.label)._indices
            ...
            #    self.annotations = {}
                ...
                for task_conv in task_converters.values():
                    ...
                    task_conv.save_annotations(item , label_data)
